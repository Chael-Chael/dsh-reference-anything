...
#### ☁️ @Cloud drive files —— 自己网盘里的文本文件

输入 `@drive:`，或者 `@baidu:`、`@pds:`、`@cloud:`、`@网盘`，即可搜索存在网盘里的文件并直接引用，不必先下载到工作区。百度网盘和阿里云盘（PDS）都已支持；不指名某一个时，两者会被同时搜索。

Reference Anything 直接调用网盘自身的 REST API，而不是转去调 CLI：`@` 菜单每敲一个键就发一次查询，"每个按键起一个子进程"这种预算并不存在。凭据复用官方 skill 已经登录好的那一份，插件自己不另存一份；从未登录过的网盘只会把自己报成不可用，所以在你主动接入之前，这个分组始终是静默的。

```text
@[百度网盘·quarterly-notes.md](dsh-ref:<opaque-base64url>)
```

**接入方式（百度网盘）**：安装官方 [`baidu-drive`](https://github.com/baidu-netdisk/bdpan-storage/tree/main/skills/baidu-drive) skill 并执行一次它的 `login.sh`，会在 `~/.config/bdpan/config.json` 生成凭据。本分组只读取该文件，不写入、不打日志，也不会把它出现在任何一条错误信息里。

**接入方式（阿里云盘 / PDS）**：安装 `aliyun` CLI 的 `pds` 插件并执行一次 `aliyun pds login`，凭据落在 `~/.aliyun/pds_config.json`，读取方式与上面完全相同。如果那里存了多个 profile，本分组用的是 `current` 指定的那一个——也就是 CLI 自己会操作的那一个。

> [!IMPORTANT]
> **百度网盘把应用限制在 `/apps/bdpan/` 之内。** 这是百度自己的沙箱，不是插件的限制：通过 `bdpan` 申请到的 token 根本看不到网盘的其余部分。所以这个分组列出的只是你放在 `我的应用数据/bdpan`（API 里写作 `/apps/bdpan/`）下面的东西。网盘里文件很多而分组却是空的，通常说明文件不在这个目录里，而不是列举失败。想再收窄，可以把 `root` 设成它下面的某个子目录。

> [!IMPORTANT]
> **阿里云盘没有这样的沙箱，问题反过来了。** 一个 PDS token 能看到它所属网盘的全部内容。`root`（或 `roots` 里针对该网盘的那一项）限制的只是**目录列举**——填一个文件夹 id，或者填一个绝对路径由本分组解析成 id；但搜索按设计仍是全盘的，因为 PDS 的搜索只能按直接父目录过滤：把它限制到某个文件夹，等于悄悄丢掉所有更深层的命中，而不是诚实地收窄结果。所以空查询列的是你指定的那个文件夹，而一旦你输入了关键词，搜索范围就是你名下的全部文件。

**读取先要正文，实在拿不到才退而取摘录。** 一次读取会按字节范围请求前 `maxReadBytes`（默认 64 KiB）。如果网盘不按范围返回、直接给了整个文件，provider 会察觉并永久降级，同时继续守住上限，而不是把几 GB 的正文照单全收；这一档不是写死的，第一次带 Range 的请求就会定下来。只有在正文完全取不到时，才会退回网盘搜索索引抽出的那段文本——返回的内容里会明确标注它是 provider 选出的摘录而非文档本身，因为用搜索片段回答"读一下这个文件"，回答的是另一个问题。两种情况下，被截断的读取都会如实报成 partial，而不是悄悄裁掉。
...
> [!NOTE]
> **夸克网盘不在计划内。** 它既没有目录列举也没有范围读取，而且它的 skill 明确禁止读取那个本该被移植的 bundle。上面两个网盘就是本分组会说的全部方言；如果 `drives` 里写了一个当前构建没有对应传输层的名字，会直接报启动错误，而不是给你一个静默的空分组——所以写错了会立刻知道。
