...
- **2026-09-03 · v0.4.0** — **适配版本：DSH `v0.1.2-rc.1` 或更高版本。** 完成 DSH 0.1.2 迁移，并将 alpha 系列转为稳定版。

  **Important**

  本版本仅适用于 DSH `v0.1.2-rc.1` 或更高版本，不兼容 DSH `v0.1.0-rc.8 ~ v0.1.1-rc.2`。旧版 DSH 用户请继续使用 `0.3.x` 稳定版。

  ### DSH RC1 兼容性

  - **迁移至 RC1 Skills Remote** —— 使用 `ctx.remote.skills.list` 及 RC1 的 `remote.skills` 注入。
  - **恢复历史引用授权** —— 会话恢复后通过 `Session.snapshotEvents()` 重建任务授权。
  - **更新 DSH SDK 依赖范围** —— peer 和 development 依赖统一要求 `>=0.1.2-rc.1`。
  - **修复插件启动兼容性** —— 将 profile 中的 `dshmarket` 和 `dsh-context` 更新至兼容 RC1 的版本。

  ### 引用菜单与来源

  - **保留自定义 `@` 菜单** —— Commands、Skills、文件、DSH 会话、本地 Agent 对话、网页对话和网盘文件仍可独立配置。
  - **保留来源图标与 Composer 胶囊** —— 文件类型、平台 Logo 和原生引用胶囊继续在各来源中正常显示。
  - **保留网盘浏览** —— OpenList 文件夹和文件继续保留来源及类型元数据。

  ### 质量改进

  - 使用真实 RC1 Session 对象增加历史授权恢复和未提及引用拒绝的回归测试。
  - 更新设置页和客户端测试以适配 RC1 接口。
  - 在真实 DSH `v0.1.2-rc.1` Web 界面完成加载与交互验证。
  - 完整验证通过：**772 项测试通过，7 项跳过**。
