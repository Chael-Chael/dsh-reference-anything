...
输入 `@drive:`，或者 `@baidu:`、`@cloud:`、`@网盘`，即可搜索存在网盘里的文件并直接引用，不必先下载到工作区。
...
> [!NOTE]
> **阿里云盘（PDS）已预留但尚未实现。** `pds` 这个名字已经进入网盘词表，这样以后补上时既不会影响任何已有引用，也不会影响已保存的设置；但现在把它写进配置会直接报启动错误，而不是给你一个静默的空分组。夸克网盘则不在计划内：它既没有目录列举也没有范围读取，而且它的 skill 明确禁止读取那个本该被移植的 bundle。
